...
# Или если requirements.txt нет, установите вручную:
pip install fastapi uvicorn python-multipart torch music21 mido pygame
```

### 3. Подготовка данных
Убедитесь, что в папке `Data/outputs/` есть:
- `midi_transformer_final.pt` - обученная модель
- `midi_transformer_final_1.pt` - копия модели (используется в коде)

Если файлов нет, скопируйте существующий:
```bash
copy "Data/outputs/midi_transformer_final.pt" "Data/outputs/midi_transformer_final_1.pt"
```
